...
generate-dev-ssl-certificate: ## Generate SSL certificate for Nginx local dev
	@if [ ! -d docker/nginx/ssl ]; then \
		mkdir -p docker/nginx/ssl; \
	fi
	@openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
	-keyout docker/nginx/ssl/investpulse.local.key \
	-out docker/nginx/ssl/investpulse.local.crt \
	-subj "/C=US/ST=State/L=City/O=Organization/CN=investpulse.local" \
	-addext "subjectAltName = DNS:investpulse.local"
